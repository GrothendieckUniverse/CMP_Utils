...
function gen_eigensys_list_along_k_path(hk::Function; kpath::Vector{<:Vector}, nk::Int=50, nband::Int64=10)::Tuple{Vector{Vector{Float64}},Vector{Matrix{Complex{Float64}}}}
    ...
    if is_sparse
        sparse_eigsys_list = [Arpack.eigs(hk(k); nev=nband, which=:SR) for k in full_kpoint_list] # usage of specifying `ClosestTo(λ)` can be found in `https://jutho.github.io/KrylovKit.jl/v0.1/man/eig/`

        eigvals_list = [sparse_eigsys[1][1:nband] for sparse_eigsys in sparse_eigsys_list]
        eigvecs_list = [sparse_eigsys[2][:, 1:nband] for sparse_eigsys in sparse_eigsys_list]
    else
